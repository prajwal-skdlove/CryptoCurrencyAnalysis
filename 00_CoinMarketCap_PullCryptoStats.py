...
def coinmarketcap_coins(n):
    '''This function pulls all the cryptocurrencies and its related statistics from the table that you see 
       in the first page of coinmarketcap.com. This doesnt pull the history of a coin
       Input number of pages worth of cryptocurrency to pull'''
            
    df_coindata = pd.DataFrame() #create an empty dataframe to later add coins to
    ...
    for pages in range(1, n + 1):
        ...
        # Each Crypto stats is in rows with the order following keyattributes/colnames
        listings = coin_data['props']['initialState']['cryptocurrency']['listingLatest']['data']
        
        
        # Loop through it to match the keysAttribute to data
        # Assign it and create a dicitonary for each coin
        # Is there a better way of doing it? IDK Just my first few weeks of dedicated python coding
        coindata_cmb = { "data":[]}
        ...
        # You have each key as keyattributes/colnames and values for each coin in a list
        ...
        #Concatenate all the data together to have a big dataset of all coins and its stats
        df_coindata = pd.concat([df_coindata,df_coins], axis = 0, sort=False, ignore_index=True)
                
    
    # There is way too many variables. Not all of them is useful, but keep them all
    # Some variables that helps to figure out what you are looking at, reorder them by bringing in upfront
    # Risk - if coinmarketcap changed the names of these variables, when it errors out. But should be relatively easy to fix
    var_first = ["rank","cmcRank", "id", "name", "symbol", "slug", "isActive", "isAudited", "dateAdded", 
                 "lastUpdated","quote.USD.price", "ath", "atl", "high24h", "low24h","circulatingSupply", "maxSupply",
                 "totalSupply", "quote.USD.marketCap", "quote.USD.marketCapByTotalSupply", "quote.USD.fullyDilluttedMarketCap"]
      
    var_order = var_first + list(set(list(df_coindata)) - set(var_first))
    
    # Fianl dataframe to return
